...
one implementation task with no repair round or explicitly requested architecture
consultation.
...
| User explicitly requests architecture consultation | Configured Astra / high consultant assesses the design read-only and returns recommendations, alternatives, consequences and unknowns; orchestrator records conclusions in spec before implementation; required checks and independent assessments remain |
| User suggests or accepts a plan without requesting architecture consultation | No architecture consultant is spawned; normal risk routing and verification apply |
...
| Ordinary application feature | Standard risk, explicitly accepted short plan; two helpers total: Sol / medium implementer and Sol / medium reviewer; implementation includes tests/docs; reviewer uses current passing command evidence |
| Change resource authorization | High risk even for one line; explicitly accepted plan; three helpers total: Sol / medium implementer and tester, Astra / high reviewer; negative authorization paths checked independently |
...
| Resume pre-update v1 task | Original approval remains valid if effective inputs are unchanged; changed model defaults invalidate approval until accepted again; original test/review/documentation gates and agents remain required; no automatic conversion |
